...
fetch("/extensions/minecraft-java/data/versions.json", {
	...
	if(e[1].hasIcon){
		...
		img.src = `/extensions/minecraft-java/data/mods/${e[0]}/icon.png`;
		ele.append(img);
	}
	ele.append(document.createElement("br"));
	const descEle = document.createElement("a");
	descEle.href = `/extensions/minecraft-java/data/mods/${e[0]}`;
	...
	rev.forEach((version, i) => {
		...
		aEle.href = `/extensions/minecraft-java/data/jars/${e[0]}-${version}.jar`;
