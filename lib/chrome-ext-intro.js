...
fetch("/chrome-extensions/versions.json", {
	cache: "reload"
}).then(response => response.json()).then(versions => Object.entries(versions).filter(e => e[0]).forEach(e => {
	...
	descEle.href = `/chrome-extensions/${e[0]}`;
	...
	aEle.href = e[1].href;
	aEle.innerText = `Download (version ${version})`;
	aEle.target = "_blank";
