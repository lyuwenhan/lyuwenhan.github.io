...
let time = Math.max(1, Number(window.localStorage.getItem("cps-time")) || 5);
let startTime;
let timeWent = 0;
let left = 0;
let started = false;
...
function restart() {
	...
	timeWent = 0;
	clicked = 0;
	testText.innerText = "Press space to start testing";
	cps.innerText = ``;
	timeline.innerHTML = "";
	if (endTo) {
		clearTimeout(endTo);
...
testTime.addEventListener("input", () => {
	...
	window.localStorage.setItem("cps-time", time);
	restart()
});
restart();
...
window.addEventListener("keydown", e => {
	...
	if (started) {
		...
		endTo = setTimeout(() => {
			...
			cps.innerText = `Your Spacebar Speed: ${to2(clickedAtEnd/time)}`
		}, time * 1e3);
		endInt = setInterval(() => {
			timeWent = (performance.now() - startTime) / 1e3;
			left = time - timeWent;
			...
			if (clicked >= 1) {
				cps.innerText = `Current Spacebar Speed: ${to2(clicked/timeWent)}`
			}
		}, 100)
	}
	if (left > 0) {
